...
export interface components {
    schemas: {
        ...
        ParamDef: {
            ...
            /**
             * Doc
             * @description 이 파라미터가 무엇인지. MCP 툴 `inputSchema` 의 `description` 이 되고 (§12.3), 프론트의 위젯 툴팁이 된다. 선택이다 — 이름과 타입만으로 충분한 파라미터가 있다
             */
            doc?: string | null;
        ...
        SubgraphDef: {
            /**
             * Doc
             * @description 이 정의가 무엇을 하는지 — **정의의 자기 서술**이다. MCP 툴 설명 (§12.2) · 프론트 위젯 툴팁 · 캔버스의 서브그래프 노드 표시가 이것을 읽는다. 서브그래프 일반에서는 선택이지만 **카탈로그에 노출되려면 필수다** (§12.8)
             */
            doc?: string | null;
            /**
             * Nodes
