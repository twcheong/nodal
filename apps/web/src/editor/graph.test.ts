...
describe("캐논 그래프와 캔버스 투영", () => {
  it("링크를 별도 포맷 없이 React Flow edge로 투영한다", () => {
    ...
    expect(graph.nodes && Object.values(graph.nodes)[2]?.inputs?.a).toHaveProperty("$link");
  });

  it("좌표가 없는 문서도 ui 필드 안에서만 보완한다", () => {
    const graph = normalizeGraph({
      nodal_version: "1",
      nodes: { a: { type: "math.Const", inputs: { value: 1 } } },
    });
    expect(graph.ui?.a).toEqual({ pos: [40, 60] });
    expect(graph.nodes?.a).toEqual({ type: "math.Const", inputs: { value: 1 } });
  });
