...
export const GraphCanvas = forwardRef<CanvasHandle>(function GraphCanvas(_props, ref) {
  ...
  const projection = useMemo(
    () => graphToFlow(graph, schemaMap, runtime, issues, connection, new Set(selectedNodeIds), nodeMeasurements),
  ...
  const sourceIntent = useCallback(
    (_event: MouseEvent | TouchEvent, { nodeId, handleId, handleType }: OnConnectStartParams) => {
      ...
      const socket = schemaMap.get(node?.type ?? "")?.outputs?.find((item) => item.name === handleId);
  ...
  const validConnection = useCallback(
    (candidate: Edge | Connection) => {
      if (!candidate.source || !candidate.target || !candidate.sourceHandle || !candidate.targetHandle) return false;
      const sourceNode = graph.nodes?.[candidate.source];
      const targetNode = graph.nodes?.[candidate.target];
      const output = schemaMap.get(sourceNode?.type ?? "")?.outputs?.find((item) => item.name === candidate.sourceHandle);
      const input = schemaMap.get(targetNode?.type ?? "")?.inputs?.find((item) => item.name === candidate.targetHandle);
  ...
  return (
    <section
      ...
      onDrop={(event) => {
        ...
        if (schema) addNode(schema, flow.screenToFlowPosition({ x: event.clientX, y: event.clientY }));
      }}
    >
      <ReactFlow
