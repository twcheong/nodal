...
- **여러 장 일괄 저장은 미지원이다.** 샘플러는 이미지 배치를 생성할 수 있지만,
  Save Image는 한 장만 저장한다. 여러 장 입력은 오류로 안내한다.
- **MiniMax H3 영상 생성은 실험적 지원이다.** 텍스트·시작/끝 이미지 입력, 소리 포함 영상 저장·재생 경로가 있다.
  원격 GPU 실측은 아직이며 [H3 설치·실행 안내](docs/minimax-h3.md)를 따른다. Intel XPU 가속은 미지원이다.
