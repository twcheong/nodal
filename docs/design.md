...
6. **확장은 매니페스트로 관리한다.**
7. **그래프 상태의 단일 소스는 서버다.** 클라이언트(웹 캔버스든 MCP 클라이언트든)는 복제본을 들고 있을 뿐 소유하지 않는다. 클라이언트가 하나였을 때는 구분할 이유가 없었지만, 지금은 **둘 이상**이다 (§7 · §12.5).
...
> ⚠️ **웹 클라이언트는 그래프 상태를 소유하지 않는다** (§2 원칙 7). 위 스토어는
> 편집 중인 **복제본**이고, 권위 있는 사본은 서버에 있다. 클라이언트가 하나였던
> M2~M4 에서는 둘의 구분이 무의미했지만, MCP 클라이언트가 같은 그래프를 건드리는
> 순간 "누가 진짜인가"가 답해져야 한다.
>
> **지금 요구하는 것은 이 방향을 막지 않는 것뿐이다.** 실시간 가시화(외부 AI 가
> 그래프를 바꾸는 것을 캔버스에서 보는 것)는 MVP 밖이다 (§12.5). 기능은 미루되
> 브라우저만 아는 상태를 늘리지 않는다 — 늘려 두면 그때 아키텍처를 뒤집어야 한다.
...
5. **저수준 `execute_graph` 툴을 함께 노출할 것인가** → 템플릿이 없는 요청의 탈출구지만, 있으면 클라이언트가 그쪽으로 흐른다 (§12.2)
6. **서브그래프를 무엇으로 펼치나** — 로드 시 평탄화 vs 실행 시 노드 확장(§5.2) → 후자면 "노드 확장" 은 강등할 수 없다. M5 착수 전에 정한다
7. **템플릿 카탈로그는 어디에 사나** — 파일 디렉토리(`~/.nodal/templates/*.nodal.json`) vs 확장 매니페스트(§8)의 일부 → 후자면 서드파티 템플릿 배포가 M6 확장 시스템에 얹힌다
8. **긴 실행을 MCP 에서 어떻게 다루나** — 툴 호출 하나가 수십 초~수 분이다. 동기 블로킹 vs `run_id` 반환 후 폴링 → 프로토콜 제약 확인 필요
9. **MCP 서버가 어느 마일스톤인가** → 아직 배치되지 않았다 (`roadmap.md`). M5(서브그래프)에 의존하므로 그 뒤이지만 M6(확장·배포)과의 순서는 사용자 결정

---

## 12. MCP 인터페이스 — nodal 의 1차 표면

**이 절은 방향 문서다. 아직 계약이 아니다.** 구현은 시작되지 않았고 (`roadmap.md`
에 마일스톤이 배치되지 않았다 — §11 열린 질문 9), 툴 이름과 파라미터 형태는
...
저수준 `execute_graph` 를 **함께** 노출할지는 열지 않았다 (§11 열린 질문 5).
