...
- 저장 위치는 현재 브라우저 origin의 OPFS `nodal/revisions/` 다. 파일마다 완전한
  ...
  **500ms 디바운스**하며 수동 저장의 다운로드 파일은 별개다
...
- 시작할 때 최신의 구조적으로 유효한 리비전을 복구한다. 마지막 파일이 쓰기 중
  ...
  브라우저 크래시 복구를 보장한다
- 보장하지 않는 것: 디바운스 500ms 안의 미저장 변경, 브라우저 프로필·site data
  삭제, origin 변경, 시크릿 모드 종료, 저장 공간 회수, 다른 컴퓨터·브라우저로의
  동기화, 서버/MCP 그래프와의 병합. OPFS가 없는 브라우저에서는 명시적으로 자동
  저장 불가를 알리고 메모리 상태를 파일인 척하지 않는다
