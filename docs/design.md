...
**`id` 는 “이 문서가 무엇인가”이지 “이 실행이 무엇인가”가 아니다.** 같은 문서를
여러 번 실행해도 `id` 는 바뀌지 않는다. 실행을 가리키는 것은 `run_id` 다 (§12.6).
...
- **Zustand** — 그래프 스토어
- **Yjs** — undo/redo의 편집 히스토리. Yjs 문서가 두 번째 그래프 포맷은 아니다
- 자동 저장 — 변경 시 디바운스 후 캐논 `.nodal.json` 리비전 저장

> ⚠️ **웹 클라이언트의 그래프 상태 소유 — 현재와 목표가 다르다.** 아래 두 문단은
> 서로 다른 시제다. 섞어 읽지 말 것.
>
> **현재 (2026-08-29 기준, 코드가 이렇다):** 권위 있는 사본은 여전히
> **브라우저 Zustand 에 있다.** OPFS 자동 저장은 크래시 복구 사본이고 내보내기는
> blob 다운로드다. 서버에는 그래프 저장소가 **없다** — `/api/graph/*` 는
> `validate` 와 `from-png` 뿐이다. OPFS 리비전을 새 권위 저장소로 간주하지 않는다.
>
...
**M7.4 undo/redo 범위 (2026-08-29)**:

- Yjs `Doc` 은 캐논 `GraphDocument` 하나의 편집 이력만 표현한다. Yjs 전용 그래프
  스키마·저장 포맷은 없고, 실행·저장·전송은 계속 같은 `.nodal.json` 이다
- 노드·링크·입력과 함께 **노드 위치 같은 문서 저작 UI도 undo 대상**이다. 배치는
  사용자가 만든 결과라 되돌려야 한다. 반대로 팬·줌 `viewport` 는 탐색 상태라
  히스토리에서 제외한다 — Ctrl+Z 가 방금 옮긴 노드 대신 화면 이동만 되돌리면 안 된다
- 실행 중에도 편집과 undo/redo를 허용한다. 서버에 이미 제출한 실행은 제출 시점의
  캐논 그래프 스냅샷으로 끝까지 돌고, 이후 편집은 다음 실행에 쓰인다. 로컬 Yjs는
  서버 실행을 수정하거나 그래프 권위를 주장하지 않는다. §12.4의 서버 그래프
  저장소가 생기면 서버가 보낸 문서를 새 기준선으로 삼아 로컬 이력을 재설정한다
- 실행 성공 뒤 시드 전환(`increment`·`randomize`)은 다음 실행 입력을 바꾸는 문서
  편집이므로 undo 대상이다

**M7.4 자동 저장 리비전 (2026-08-29)**:

- 저장 위치는 현재 브라우저 origin의 OPFS `nodal/revisions/` 다. 파일마다 완전한
  캐논 `.nodal.json` 하나를 쓰고, 전체에서 최신 **20개**를 보관한다. 변경 후
  **500ms 디바운스**한다. 문서가 숨겨지는 `visibilitychange(hidden)` 에서 대기 중인
  저장을 즉시 시작하고 `pagehide` 에서 한 번 더 시도한다. 수동 저장의 다운로드
  파일은 별개다
- M7.4 이전의 `localStorage["nodal.lastGraph"]` 300ms 사본은 업그레이드 때 한 번
  읽어 검증하고 OPFS 파일로 옮긴다. OPFS 쓰기가 완료된 뒤에만 옛 키를 삭제하며,
  깨진 값이나 쓰기 실패는 삭제하지 않는다
- 시작할 때 최신의 구조적으로 유효한 리비전을 복구한다. 마지막 파일이 쓰기 중
  깨졌다면 그보다 앞선 유효 파일로 물러난다. 따라서 **쓰기 완료된 리비전까지**의
  브라우저 크래시 복구를 보장한다
- 보장하지 않는 것: `visibilitychange`/`pagehide` 저장이 완료되기 전에 브라우저나
  OS가 프로세스를 강제 종료한 경우의 마지막 변경, 브라우저 프로필·site data 삭제,
  origin 변경, 시크릿 모드 종료, 저장 공간 회수, 다른 컴퓨터·브라우저로의 동기화,
  서버/MCP 그래프와의 병합. OPFS가 없는 브라우저에서는 명시적으로 자동 저장 불가를
  알리고 메모리 상태를 파일인 척하지 않는다
...
├── nodal.toml               # 매니페스트
├── nodes/*.py                # @node 데코레이터
├── templates/*.nodal.json    # 선택: §12.8 카탈로그에 합류하는 템플릿
└── web/                       # 선택: ESM 전송로만 있음. 커스텀 위젯 API는 아직 없음
    ├── index.js               # 엔트리 — 있어야 web_entry_url 이 null 이 아니다
    └── *                       # 엔트리가 import 하는 형제 파일. 서브트리 전체를 낸다
```
...
name = "My Pack"
version = "0.2.0"
nodal_api = "^0.1"        # 이거 없으면 로드 거부

[dependencies]
python = ["opencv-python>=4.9"]  # 예약 형식. 현재 자동 설치 구현은 없음
```
...
- **현재 (M7.5)**: `[dependencies].python` 은 예약 형식이지만 로더와 런처가 읽어
  설치하지 않는다. nodal 환경에 이미 없는 추가 패키지를 요구하는 확장은 자동 설치할
  수 없다. **목표**는 설치 시점에 `uv` 로 의존성을 해석·충돌 검출하는 것이다 →
  "커스텀 노드 설치했더니 torch 버전이 깨짐" 회피. 실행 시점에는 프로세스를
  격리하지 않고 확장을 `nodal serve` 와 **같은 프로세스로 import** 한다
  (2026-08-26, `decisions.md`). 진짜 실행 시 격리는 노드 실행을 IPC 로 만들고
  `Image`(§4.4)를 프로세스 경계로 직렬화해야 하며, `Model`·`CLIP` 같은 불투명
  핸들(§9.1)은 애초에 프로세스 경계를 넘지 못한다 — §12.3 이 "핸들은 엔진
  안에서만 산다" 고 적은 것과 부딪친다
- 프론트 확장은 ESM 동적 import. 백엔드 패키지에 섞지 않는다

**M7.2 구현 (`nodal.extensions`, `nodal_server.app`, `nodal_nodes_core.cli`)**:

- `nodal_api` 범위 불일치 → 로드 거부 (경고가 아니라 거부). 지원하는 범위 문법은
  **캐럿(`^`) 뿐**이다 — `>=`·`~`·콤마 범위는 명확한 에러로 거부한다. 이 서버가
  구현하는 확장 API 계약 버전은 `nodal.NODAL_API_VERSION` 이고, 패키지 릴리스
  버전(아직 `0.0.0`)과는 별개다. 현재 계약은 `0.1.0` 이며 `^0.1` 은
  `>=0.1.0, <0.2.0` 이다 — 0.x 에서는 minor 변경이 호환성 경계라 0.2 로 올리면
  0.1 확장은 의도적으로 거부된다. 확장 표면과 호환성 정책이 안정되어 제3자가
  브레이킹 변경 없는 사용을 기대할 수 있을 때 `1.0.0` 으로 올린다
- 확장 하나의 예외가 나머지 확장의 로드를 막지 않는다 — `nodes/*.py` 를 import
  하다 던진 예외를 포함한다. 한 확장의 노드는 전부 import 하고 스키마 충돌까지
  검증한 뒤 실제 레지스트리에 한 번에 반영한다. 실패한 확장의 노드 일부가 남지 않는다
- 실패는 `GET /api/extensions` 응답의 `failed[]` 로 나간다 — 성공적으로 로드된
  목록과 같은 응답에서 구분된다
- 노드 등록은 `NodeRegistry.register` 하나뿐이다. `@node` 가 붙인
  `__nodal_schema__` 마커로 클래스를 찾아 그 경로에 흘려보낸다 — 두 번째 등록
  경로를 만들지 않는다
- `run`·`validate`·`serve`·`nodes` 네 명령이 전부 같은 기본 경로
  (`~/.nodal/extensions`)를 자동으로 보고, `--extensions DIR` 로 같은 사용자 지정
  경로를 재현할 수 있다
- 확장이 `templates/*.nodal.json` 을 들고 오면 §12.8 의 같은 카탈로그에 합류한다.
  **충돌 규칙**: 사용자 디렉토리(`~/.nodal/templates`)가 언제나 이기고, 확장끼리
  겹치면 먼저 로드된(= id 순으로 앞선) 쪽이 이긴다. 진 쪽은 조용히 사라지지 않고
  거부 사유에 누가 이겼는지 남는다 (2026-08-27, `decisions.md`)

**프론트 확장 엔트리 — URL 은 서버가 준다 (2026-08-27 계약)**:

`ExtensionInfo.web_entry_url` 은 그 확장의 `web/index.js` 를 낼 **완성된**
`/api/...` 경로다. `web/index.js` 가 없으면 `null`. **프론트는 이 문자열을
그대로 `import()` 한다 — `id` 로 URL을 조립하지 않는다.** URL 규칙(경로 모양·
접두)이 프론트와 백엔드 두 곳에 각자 표현되면, ComfyUI MCP 브리지를 거부한
이유(§12.1 — "서버가 이미 아는 것을 두 번 표현하면 어긋난다")와 같은 방식으로
어긋난다. 서버가 라우트를 바꿔도 프론트는 고칠 것이 없어야 한다.

**M7.3 은 전송로를 만들었고 모듈 평가까지만 한다. 프론트 확장 API 표면은 아직
없다.** 서버가 조립한 `web_entry_url` 을 프론트가 그대로 동적 `import()` 하고,
모듈은 한 번 평가되며, 백엔드의 `failed[]` 와 프론트 평가 실패는 같은 UI 배너에
모인다. 이 동작은 이미 구현된 전송 계약이다. 반면 평가된 모듈이 호출할 export·
등록 함수·위젯 마운트와 해제 수명주기는 정의되지 않았다. 따라서 커스텀 위젯은
**아직 저작할 수 없고**, `globalThis` 나 임의 DOM 부수효과를 사실상의 호환성
계약으로 사용하지 않는다. API 표면을 여는 조건은 §11 열린 질문 20에 둔다.

- **URL 은 `/api/` 아래에 둔다.** `vite.config.ts` 가 이미 `/api` 를
  `127.0.0.1:8188` 로 프록시하므로 dev 모드가 추가 설정 없이 돈다. 별도
  정적 마운트(`app.mount("/ext-static", ...)` 류)를 만들면 프록시 설정이
  **두 번째 규칙**이 된다 — 하나만 둔다
- **`index.js` 하나가 아니라 그 확장의 `web/` 서브트리 전체를 낸다.**
  `GET /api/extensions/{id}/web/{file_path:path}` 가 그 서브트리 안의 임의
  파일을 돌려준다. ESM 엔트리는 형제 파일을 `import` 하므로, 엔트리만 내면
  확장 저자가 파일을 하나 이상으로 쪼개는 순간 깨진다
- **경로 탈출을 막는다.** 요청 경로를 확장의 `web/` 밑에 이어 붙인 뒤
  `resolve()` 하고, 그 결과가 `web/`(역시 `resolve()` 한 것) 의 하위인지
  확인한다. `..` 세그먼트도 심볼릭 링크로 밖을 가리키는 것도 이 한 번의
  비교로 걸린다 — `resolve()` 가 링크를 따라가므로 결과가 `web/` 밖이면
  무조건 거부된다
- **`loaded=True` 인 확장만 낸다.** 실패한 확장의 파일을 내보내면 UI 배너("이
  확장은 로드되지 않았다")와 실제로 파일이 내려오는 것이 어긋난다.
  `failed[]` 의 id 로 요청해도 404 다
- **`.js`·`.mjs` 는 `Content-Type: text/javascript` 를 강제한다.**
  `mimetypes` 의 플랫폼 등록에 맡기면 `application/javascript` 가 나올 수
  있고, 그러면 브라우저가 ESM 으로 실행하지 않고 조용히 죽는다. 다른 파일은
  표준 추측(`mimetypes.guess_type`)에 맡긴다

**라이선스 시야의 한계**: 서드파티 확장은 GPL 일 수 있다 — `~/.nodal/extensions`
는 이 저장소 밖(사용자 홈 디렉토리)이라 배포물에 들어오지 않으므로 절대 규칙 1
위반이 아니다. 다만 CI 의 `licence-guard` GPL grep 은 **이 저장소 트리만** 본다
— 확장이 그 시야 밖에 있는 것은 당연한 결과이지 이 검사가 "시스템에 GPL 코드가
없음"을 보장한다는 뜻이 아니다 (2026-08-27, `ci.yml` 의 GPL 검사 주석도 같은
내용을 적는다).
...
> **"VRAM 압박 처리"는 위 표현이 실제 코드보다 똑똑하게 들린다.** `resolve_plan`
> (`devices.py:194`)은 여유 VRAM 을 보지 않는다 — CUDA 면 `supports_offload=True`
> 이므로 **압박과 무관하게 항상 오프로드한다** (`devices.py:211`). `Backend` 에
> `reports_free_memory` 와 `free_memory()` 가 있는데도 이 판단에 쓰지 않는다.
>
> 이것을 결함으로 적지 않는다 — **개발 장비(RTX 5080, 15.92 GiB)에서는 옳은
> 결과였다.** SDXL 파이프라인 2개를 이 카드에 동시에 상주시킬 수 없고, 오프로드
> 덕에 `live_pipelines=2` 상태에서도 allocated 가 0.35 GiB 로 유지된다(실측,
> `decisions.md` 2026-08-25). 다만 **그 판단이 근거가 아니라 우연**이라는 것은
> 적어 둔다 — 여유 VRAM 이 넉넉한 카드에서는 항상 켜진 오프로드가 불필요한
> 비용(프리뷰 기준 실측 2배, §10)을 치르게 만들 수 있다. 여유를 보고
> 판단해야 하는가는 아직 답하지 않았다(§11 열린 질문 19).
...
| VRAM 관리를 `accelerate`에 맡긴 경로의 실제 비용 | 정상 상태를 실측했다(RTX 5080, `live_pipelines=2`, 1024², `steps=60`): 프리뷰 켬 35.66초·max allocated 7.22 GiB, 프리뷰 끔 17.48초·max allocated 7.14 GiB — 배수 **2.04**. 프리뷰는 §9.5의 실제 VAE 디코드라 공짜가 아니다. 이전에 이 행에 적혀 있던 22분56초(스텝당 23초, 70배)는 그 실행의 allocated가 16.59 GiB로 **카드 물리 총량(15.92 GiB)을 넘은 예외 상태**였다 — 정상 경로가 아니다. 그 상태를 만든 조건은 후속 조사에서 재현 시도 5회(in-process·실서버·`live_pipelines=2` 고정 등 조건을 바꿔 가며) 모두 실패해 **원인 미상으로 남긴다**(§11 열린 질문 18, `decisions.md` 2026-08-25). 추측을 사실처럼 적지 않는다 |
...
13. ~~평탄화된 그래프를 UI 에서 다시 접을 것인가~~ → **해소됨 (2026-08-29,
    M7.4). 지금은 접지 않는다.** 프론트 편집 상태와 리비전은 평탄화 전 캐논 문서고,
    평탄화 결과는 검증·실행 안에서만 산다. UI에 들어오지 않는 표현을 되접는 기능은
    undo/복구에 필요하지 않다. `<인스턴스>:<안쪽>` 접두 계약(§5.5)은 오류 귀속과
    실행 추적을 위해 그대로 유지한다. 나중에 평탄화 결과를 UI가 실제로 받게 되면
    그때 별도 기능으로 결정한다
...
16. ~~MCP 클라이언트가 이미지를 어떻게 건네나~~ → **해소됨 (형식 2026-08-24,
    실행 경로 2026-08-27).** `Image` 파라미터의 값은 파일 경로이거나
    `asset:<hash>` 이고 판별은 예약 접두다 (§12.3). 클라이언트 파일은 같은 서버의
    `POST /api/assets`에 올리고 응답 해시를 `asset:<hash>`로 넘긴다. 호출 래퍼가
    두 표현을 각각 `image.Load` · `image.LoadAsset` 노드로 바꾼다
...
**오프로드 후속 조사(2026-08-25)가 연 것들:**

18. **16.59 GiB(카드 총량 초과) 상태를 만든 조건은 무엇인가** — M4 재검증 B가
    관측한 예외 상태다. 재현 시도 5회(in-process·실서버, `live_pipelines=2` 고정
    포함)가 전부 정상 속도로 끝나 원인을 좁히지 못했다. 세 가설(VRAM 압박이
    오프로드를 켠다 · 오프로드×프리뷰 · 상주 2개+프리뷰 피크의 합)은 순서대로
    반증됐다 — 자세한 경위는 `decisions.md` 2026-08-25
19. **`resolve_plan` 이 여유 VRAM 을 근거로 오프로드 여부를 판단해야 하는가** —
    지금은 CUDA 면 무조건 켠다(§9.6). 개발 장비에서는 우연히 옳았을 뿐, 여유가
    넉넉한 카드에서는 프리뷰 기준 실측 2배 비용을 근거 없이 치른다. `Backend` 에
    이미 `free_memory()` 가 있다 — 쓸지 말지는 M6 이후 판단
20. **프론트 확장 API 표면(export·등록·위젯 수명주기)은 무엇인가** — M7.5 에서
    **지금 정하지 않기로 했다.** M7.3 의 URL·ESM 평가·실패 배너 전송로는 유지하지만
    커스텀 위젯 저작은 아직 지원하지 않는다. **실제 커스텀 위젯이 필요한 확장이
    하나 생기면**, 그 구체적 요구에서 최소 API와 마운트·해제 수명주기를 뽑아 사용자
    확인 후 §8과 `decisions.md`에 확정한다. 그 전에는 `globalThis`나 임의 DOM
    부수효과를 예시·가이드·호환성 표면으로 인정하지 않는다
...
| 전송과 배치 (§12.9) · `Image` 값의 경로/`asset:` 라우팅 (§12.3) | — |
...
**두 형식이 모두 동작한다.** 호출 래퍼가 `Image` 파라미터의 문자열을 실제 이미지
링크로 바꾼다:

| MCP 값 | 호출 그래프에 생기는 노드 | 서브그래프가 받는 값 |
|---|---|---|
| 파일 경로 | `image.Load(path=...)` | 그 노드의 `Image` 출력 링크 |
| `asset:<hash>` | `image.LoadAsset(asset_hash=...)` | 그 노드의 `Image` 출력 링크 |

`image.Load`는 고치지 않았다. 경로와 content hash는 발견 규칙이 다르므로
`image.LoadAsset`가 `ctx.assets`에서 바이트를 읽는다 (§4.5). 접두는 호출 래퍼에서
한 번만 판별하고 노드에는 순수 해시만 들어간다. 따라서 임시 경로가 생기지 않고
같은 `asset:<hash>` 재호출의 캐시 키도 안정적이다 (§5.3).

클라이언트 파일은 **같은 서버의 `POST /api/assets`**에 multipart 필드 `file`로
올린다. 응답의 `hash`를 `asset:<hash>`로 `Image` 파라미터에 넘긴다. 별도 MCP
업로드 툴은 없다 — 바이너리를 JSON/base64로 다시 실으면 결과 바이트를 툴 응답에
싣지 않는 위 원칙과 같은 컨텍스트 비용이 생기고, §12.2의 템플릿 단위 툴 표면도
넓어진다. 이 절차는 `Image` 파라미터의 `inputSchema.description`에도 실린다.

없는 해시는 구조·타입 `GraphIssue`가 아니라 **실행 중 노드 에러**다. 에셋 존재는
주입된 외부 저장소의 상태이고 캐논 그래프 자체의 성질이 아니기 때문이다. 에러는
호출 래퍼의 안정적인 노드 ID와 입력 소켓으로 파라미터를 지목한다. 예:

```
nodes.input:source.inputs.asset_hash: 에셋 해시 값 'deadbeef'을 저장소에서 찾을 수 없다
```

`asset:`는 여전히 예약 접두다. `Image`로 선언되지 않은 파라미터에서 발견하면
`invalid_asset_reference`로 실행을 만들기 전에 거부한다.
...
**툴 호출 하나는 그래프 하나로 감싸인다.** `Image` 파라미터가 없으면 `call`만 있고,
있으면 그 앞에 표현을 읽는 입력 노드가 붙는다:

```jsonc
{
  "nodal_version": "<템플릿 파일의 nodal_version>",
  "id": "<템플릿 파일의 id>",
  "definitions": { /* 템플릿 파일의 정의 전부 */ },
  "nodes": {
    "input:<Image 파라미터>": {
      "type": "image.Load 또는 image.LoadAsset",
      "inputs": { /* 경로 또는 접두를 뗀 hash */ }
    },
    "call": {
      "type": "subgraph.<템플릿 ID>",
      "inputs": { /* 일반 툴 인자 + Image 입력 노드 링크 */ }
    }
  },
  "outputs": ["call"]
}
...
호출 그래프의 `id` 는 새로 만들지 않고 **템플릿 파일의 `id` 를 그대로 쓰며**,
`nodal_version`도 파일의 값을 보존한다. 호출마다 바뀌는 실행 식별자는 `run_id` 다
(§12.6).
