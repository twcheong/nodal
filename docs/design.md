...
**M6.0 계약(§12)이 새로 연 것들** — 전부 M6 실행 경로 배선에서 답한다:

15. **툴과 파라미터의 설명을 어디서 얻나** → 지금은 선언에서 만들어 낸다 (§12.3).
    캐논 포맷의 `params` 에는 `doc` 이 없고 `SubgraphDef` 에는 제목도 없다.
    MCP 툴 설명은 AI 가 툴을 고르는 근거이므로 품질이 곧 제품 품질인데, 필드를
    더하는 것은 **캐논 포맷 변경**이라 사용자 확인이 필요하다 (`AGENTS.md` 규칙 7).
    M6 실행 단계에서 결정한다
16. **MCP 클라이언트가 이미지를 어떻게 건네나** → 지금은 `STRING` 파일 경로다
    (§12.3). 맞는 방향은 `POST /api/assets` 로 올리고 해시로 가리키는 것인데
    (§4.5), **에셋 저장소에서 읽는 노드가 없다.** 결과가 에셋 참조로 나가는 것과
    대칭이 되려면 그 노드가 필요하다
17. **`/mcp` 를 로컬 밖으로 열 때의 인증** → 지금은 없다. `127.0.0.1` 전제가
    깨지는 순간(원격 워크스테이션·컨테이너) 필요해진다. MVP 범위 밖이다

---

## 12. MCP 인터페이스 — nodal 의 1차 표면

**이 절은 계약이다** (2026-08-24, M6.0). 착수 시점에 확정하기로 했던 툴
...
| 텐서 (`Image` · `Mask` · `Latent`) | ❌ | MCP 인자는 JSON 이다. 클라이언트가 텐서를 만들 수 없다 |
...
**밖에서 이미지를 받아야 하면** `STRING` 으로 받아 템플릿 안에서 로드한다. 지금
그 자리는 파일 경로다 (`image.Load`). 클라이언트가 파일을 올리고 해시로
가리키는 쪽(`POST /api/assets` → §4.5)이 더 맞지만 **에셋에서 읽는 노드가 아직
없다** — 열린 질문 16.
...
- **인증은 아직 없다.** `nodal serve` 는 기본이 `127.0.0.1` 이고 로컬 단일
  사용자를 전제한다. MCP 를 붙였다고 그 전제가 바뀌지 않는다 — `/mcp` 를 로컬
  밖으로 여는 것은 열린 질문 17 이다
