...
6. **중첩은 재귀한다.** 안쪽 정의가 또 인스턴스를 담으면 같은 규칙을 반복한다.
   한계는 **두 가지를 함께** 둔다 (§11 열린 질문 12) — 깊이만으로는 부족하다.
   중첩 2단이어도 각 정의가 50노드면 결과가 폭발한다
...
12. **평탄화 한계를 얼마로** → **값은 M5.3 에서 정한다.** 모양은 정했다: **깊이 상한과 결과 노드 수 상한을 둘 다** 둔다. 깊이만으로는 부족하다 — 중첩 2단이어도 각 정의가 50노드면 결과가 폭발한다. 초과 시 **어느 쪽 한계에 걸렸는지** 메시지가 지목한다 ("깊이 8 초과" 와 "노드 2000개 초과" 는 사용자가 할 일이 다르다)
