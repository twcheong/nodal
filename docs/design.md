...
- `nodal_api` 범위 불일치 → 로드 거부 (경고가 아니라 거부). 지원하는 범위 문법은
  **캐럿(`^`) 뿐**이다 — `>=`·`~`·콤마 범위는 명확한 에러로 거부한다. 이 서버가
  ...
  버전(아직 `0.0.0`)과는 별개다 (2026-08-27, 스펙이 값을 정하지 않은 데 대한
  임의 해석 — `decisions.md`)
...
- 노드 등록은 `NodeRegistry.register` 하나뿐이다. `@node` 가 붙인
  ...
  경로를 만들지 않는다
- `run`·`serve`·`nodes` 세 명령이 전부 같은 기본 경로(`~/.nodal/extensions`)를
  자동으로 본다. `validate` 는 `--extensions` 로 경로를 바꿀 수는 없지만 같은
  기본 경로는 본다 — `--pack` 이 이미 `validate` 에는 없는 것과 같은 비대칭이다
