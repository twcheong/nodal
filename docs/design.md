...
- 확장은 **설치 시점**에 `uv` 로 자체 격리 환경에 의존성을 설치한다 →
  "커스텀 노드 설치했더니 torch 버전이 깨짐" 회피. **격리는 여기까지다** — 실행
  시점에는 프로세스를 격리하지 않는다. 확장은 `nodal serve` 와 **같은 프로세스로
  import** 된다 (2026-08-26, `decisions.md`). 진짜 실행 시 격리는 노드 실행을
  IPC 로 만들고 `Image`(§4.4)를 프로세스 경계로 직렬화해야 하며, `Model`·`CLIP`
  같은 불투명 핸들(§9.1)은 애초에 프로세스 경계를 넘지 못한다 — §12.3 이 "핸들은
  엔진 안에서만 산다" 고 적은 것과 부딪친다
...
**M7.3 은 모듈 평가까지만 한다. 프론트 확장 API 표면은 아직 없다.** 확장이
호출할 export·등록 함수·수명주기는 정의되지 않았으며, `globalThis` 나 임의의 DOM
부수효과를 사실상의 호환성 계약으로 굳히지 않는다. 이 표면은 M7.5 노드 저작
가이드를 쓰기 전에 별도 계약으로 먼저 확정한다.
