...
> ⚠️ **현재 구현은 위의 절반이다.** `_classify`(`executor.py:935`)가 맨몸
> 블로커만 `Blocked` 로 분류하고, 전파(`propagate_blocker`:952)와 상속
> (`_inherited_blocker`:980)은 **노드 단위**라 어느 소켓에서 왔는지를 보지 않는다.
> 소켓 단위는 M5.1b 에서 구현한다. 이 절은 목표를 적은 것이다.
...
1. **서브그래프 출력 → 부모 출력 매핑.** 확장을 소비하던 하류는 `["부모", "소켓"]`
   ...
   — §5.5 의 `SubgraphDef.outputs` 와 같은 문제이고, 같은 모양으로 푼다
...
서브그래프는 저작 편의가 아니라 **MCP 툴의 단위**다 (§12.2). `list_templates()`
가 보여주는 것과 `run_template(id, params)` 가 실행하는 것이 이 절의 구조물이다.
...
```jsonc
{
  ...
  "definitions": {
    "thumbnail": {
      ...
      "outputs": { "image": { "$link": ["fit", "image"] } }   // ← 밖으로 나가는 값
    }
  },
  "nodes": {
...
`params` 는 이름 → `{type, default, widget}` 이다.
...
`outputs` 는 인스턴스가 내보내는 소켓 이름 → 정의 안의 `(노드, 소켓)` 이다.
...
문서를 읽은 직후, 실행 엔진이 보기 전에 다음을 수행한다:
...
4. **경계 링크를 재배선한다.** 밖에서 `["thumb", "image"]` 를 읽던 링크는
   정의의 `outputs["image"]` 가 가리키는 안쪽 노드로 바뀐다 → `["thumb:fit", "image"]`
5. **인스턴스 노드를 지운다.** 평탄화가 끝난 그래프에 `subgraph.*` 타입은 남지 않는다
6. **중첩은 재귀한다.** 안쪽 정의가 또 인스턴스를 담으면 같은 규칙을 반복한다.
   **깊이 제한**을 둔다 — 순환은 검증이 잡지만(아래), 깊게 중첩된 정상 문서가
   메모리를 폭발시키는 것은 막지 못한다
...
`validate_graph`(문서 검증)가 잡는다. 노드 팩이 하나도 설치되지 않은 서버에서도
답할 수 있어야 하므로 소켓 타입 호환성(`validate_for_execution`)과 층을 나눈다.
...
10. **인스턴스가 필수 파라미터를 채웠는지 누가 보나** → 지금은 **아무도 보지 않는다.** 문서 검증은 파라미터 *타입*을 모르고(레지스트리를 안 본다), `validate_for_execution` 은 `subgraph.*` 를 모른다. 평탄화(M5.3) 시점에 "기본값도 없고 인스턴스도 안 준 파라미터" 로 잡는 것이 자연스럽지만, 그러면 **로드가 실패**하는 것이라 에러 경로가 검증과 달라진다
11. **`params[].type` 의 타입 표현식이 유효한지 누가 보나** → 문서 검증에 넣지 않았다. 타입 카탈로그를 알아야 하고, 그것을 아는 층은 `validate_for_execution` 이다. 다만 그 층은 아직 `subgraph.*` 를 모른다 — 10 과 같은 구멍이다
12. **평탄화 깊이 제한을 얼마로** → 순환은 검증이 잡지만 정상적인 깊은 중첩은 못 막는다. 값은 정하지 않았다
13. **평탄화된 그래프를 UI 에서 다시 접을 것인가** → `<인스턴스>:<안쪽>` 접두를 되읽으면 가능하다. M5.5 프론트가 필요하다고 판단하면 그때. 계약(접두 문법)은 이미 §5.5 에 있다
14. **`POST /api/graph/validate` 가 평탄화 전 문서에 무엇을 답해야 하나** → M5.3 전까지 서브그래프 문서는 `valid: false` (`unknown_node_type`)다. 엔드포인트가 평탄화한 뒤 검증할지, 문서 검증과 실행 검증을 나눠 답할지는 정하지 않았다

---

## 12. MCP 인터페이스 — nodal 의 1차 표면

**이 절은 방향 문서다. 아직 계약이 아니다.** 구현은 시작되지 않았고 (**M6** —
`roadmap.md`, 2026-08-23 사용자 결정), 툴 이름과 파라미터 형태는
착수 시점에 사용자 확인을 거쳐 확정한다. 지금 고정하는 것은 **무엇을 노출하고
무엇을 노출하지 않는가**이다.
...
```
run_template(id, params)  → 즉시 { run_id } 반환
get_run(run_id)           → 상태 · 진행률 · 결과(에셋 참조)
cancel_run(run_id)        → 취소
```
