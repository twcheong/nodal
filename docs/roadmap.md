...
- [ ] 서브그래프 노드 (그룹 접기) — **툴의 단위** (`design.md` §12.2).
      ...
      펼치는 방식(로드 시 평탄화 vs 실행 시 노드 확장)은 아직 정하지 않았다 —
      착수 전에 정한다 (`design.md` §11 열린 질문 6)
...
- [ ] 노드 확장(서브그래프 반환) + ephemeral 노드 부모 추적 (§5.2) —
      **위 열린 질문 6 이 "실행 시 확장" 으로 결론나면 ①로 올라온다.**
      그때는 강등된 채로 둘 수 없다
...
> nodal 은 MCP 서버다 (`design.md` §12). **다만 어느 마일스톤에 놓을지는 아직
> 정해지지 않았다**: M5(서브그래프)에 의존하므로 그 뒤이지만, M6(확장·배포)과의
> 순서는 사용자 결정 사항이다.
