...
**벤치마크**: 200노드 그래프에서 캔버스가 60fps 유지. — ⬜ **미측정**
계측 도구는 있다 (툴바 `200 노드 측정` → 90프레임 rAF 평균). 하지만 **수치가 어디에도
기록되어 있지 않다.** 헤드리스 브라우저에서는 탭이 백그라운드(`document.hidden`)라 rAF 가
throttle 되어 측정 자체가 불가능했다. `graph.test.ts` 는 그래프가 200노드·199엣지인 것만
확인하고 렌더 성능은 다루지 않는다.
**사람이 포그라운드 브라우저에서 버튼을 눌러 수치를 남겨야 완료다.**
