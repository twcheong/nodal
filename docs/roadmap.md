...
- [ ] `@node` 데코레이터 + 스키마 리플렉션 (`design.md` §4.2)
- [ ] 타입 시스템 + 호환성 규칙 + `types.json` 단일 소스 (§4.3)
- [ ] `TopologicalSort` / `ExecutionList` — 용해 방식 (§5.1)
- [ ] 사이클 탐지 (역방향 용해)
- [ ] 입력 시그니처 캐시, 인메모리 LRU (§5.3)
- [ ] 동기/비동기 노드 실행 (코루틴 자동 감지)
- [ ] 출력 노드 우선 선택 휴리스틱 (§1.1 ②)
- [ ] 진행률 이벤트 + 협조적 취소 (§5.4)
- [ ] CLI: `nodal run graph.json`
