...
### 2026-08-27 · Claude Code · M7.2 확장 API 버전과 범위 문법 (스펙 모호 임의 해석)

- **결정**: `design.md` §8 은 매니페스트 예시로 `nodal_api = "^1.0"` 만 주고 이
  서버가 실제로 어떤 버전을 구현하는지, 어떤 범위 문법을 지원하는지는 정하지
  않았다. 다음으로 좁혔다:
  - 이 서버가 구현하는 **확장 API 계약** 버전은 `nodal.NODAL_API_VERSION =
    (1, 0, 0)` — 패키지 릴리스 버전(`nodal.__version__`, 아직 `0.0.0`)과는
    별개다. 확장 호환성 계약은 배포 버전과 다른 속도로 움직이고, 매니페스트
    예시가 이미 `^1.0` 을 전제하므로 프리-1.0 릴리스 버전을 그대로 쓸 수 없다
  - `nodal_api` 범위 문법은 **캐럿(`^`) 만** 지원한다. `>=`·`~`·콤마 범위는
    `ValueError` 로 명확히 거부한다
  - 캐럿 파싱은 npm/Cargo 규칙(가장 왼쪽 0 이 아닌 자리 고정)을 그대로 쓴다 —
    `^1.2.3` → `>=1.2.3, <2.0.0` 처럼
- **이유**: 계약이 하나도 없으면 "로드 거부"를 구현할 기준 자체가 없다.
  프리-1.0 릴리스 버전을 그대로 API 버전으로 쓰면 첫 번째 확장 예시조차
  거부되므로, 두 버전을 분리하는 것이 유일하게 일관된 해석이다. 범위 문법을
  캐럿으로 좁힌 것은 스펙이 준 유일한 예시가 캐럿이었기 때문이고, 지원하지
  않는 문법을 조용히 다르게 해석하는 것보다 명확히 거부하는 편이 §8 의
  "로드 거부" 정신에 맞는다
...
- **결정**: `~/.nodal/extensions` 확장은 `--pack` 처럼 이름을 대야 올라오는
  ...
  자동으로** 본다. `run`·`serve`·`nodes` 는 `--extensions DIR` 로 경로를 바꿀
  수 있고, `validate` 는 플래그가 없지만(기존에 `--pack` 도 없다) 같은 기본
  경로는 본다. 확장 로드 실패는 어느 명령에서든 stderr 에 사유와 함께 찍는다
