...
### 2026-08-13 · Claude Code · M2 점검 결과 — 남은 두 가지

로드맵 M2 항목 8개는 실서버로 확인해 전부 표시했다. 남은 것 둘을 여기 적어 둔다.

- **⬜ 벤치마크 미측정**: 200노드 60fps 수치가 저장소 어디에도 없다. 계측 도구는 있지만
  (툴바 `200 노드 측정`) 헤드리스 브라우저에서는 탭이 백그라운드라 `requestAnimationFrame` 이
  throttle 되어 측정이 불가능하다. **사람이 포그라운드 브라우저에서 눌러 수치를 남겨야 한다.**
  자동화하려면 Playwright 같은 실제 렌더 환경이 필요하고, 그건 M2 범위를 넘는다.

- **🐛 연속 실행 시 노드 상태가 `예약`에 고착** (`apps/web`, 프론트 담당 영역):
  `실행` 을 빠르게 3번 누르면 서버는 3건 모두 완료하고 브라우저도 `node.cached` · `run.done` 을
  전부 수신하는데, UI 노드 배지가 `예약`(queued)에서 더 이상 바뀌지 않는다. 재현 조건은
  "이전 실행이 끝나기 전에 다시 실행"이다.
  원인 후보: `editorStore.handleEvent` 의 `activeRunId` 가드와 `startRun` 의 늦은 HTTP 응답
  처리가 겹치는 지점. 한 번씩 누르는 정상 경로에서는 `캐시` · `완료` 가 올바르게 표시된다.
  `editorStore.test.ts` 는 단건 이벤트만 재생하므로 이 경합을 잡지 못한다.
