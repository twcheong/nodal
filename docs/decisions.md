...
### 2026-08-31 · 사용자 · M6·M7 사람 완료 판정

- **결정**: `roadmap.md`의 M6 체크박스 7개와 M7에 실제로 존재하는 체크박스 6개를
  모두 완료로 판정한다. M3 후속 UX 1개와 M5 ③의 lazy 입력·노드 확장 2개는
  구현하지 않았으므로 그대로 둔다
- **M6 근거**: MCP 카탈로그·Origin·비동기 실행, 고정 시드의 동일 해시와 하류 캐시,
  경로/`asset:` 동일 이미지, 광고 템플릿 3종 결과를 함께 판정했다
- **M7 근거**: 원클릭 런처와 사용자 홈 확장 경로를 확인했고, 서드파티 팩을
  `~/.nodal/extensions`에 설치해 `/api/extensions` 로드, `/api/nodes` 33개 등록,
  프론트 검색과 캔버스 추가까지 확인했다. 이 사용자 판정은 2026-08-29 항목의
  “아직 M7 완료로 판정하지 않는다” 상태를 명시적으로 대체한다
- **영향 범위**: `AGENTS.md`, `README.md`, `docs/{roadmap,decisions}.md`
- **되돌릴 수 있나**: 예 — 완료 증거가 반증되면 해당 체크박스와 현재 단계만 되돌린다
...
### 2026-09-03: H3 Pruned diffusers 확장 클래스 연결

사용자의 경량 H3 직접 실행 요청에 따라 기존 영상 노드에 Pruned/ConvRot 프로필을 추가한다.
공개 Apache-2.0 diffusers 확장 클래스는 모델 폴더에서 해시 검증 후 로드하며 모델 계층을
직접 구현하거나 ComfyUI 코드를 도입하지 않는다. 추론은 기존 ModularPipeline에 위임한다.
Pruned 경로의 스트림 프리페치는 호스트 메모리 부담을 줄이기 위해 끈다. Windows GPU 검증 전
자동 컴파일은 적용하지 않는다. ComfyUI INT8 단일 파일 직접 로딩·동일 메모리 사용량을
지원한다고 표기하지 않는다. 고정 리비전 모델 준비 명령과 독립 Pruned 예제를 제공한다.
