...
### 2026-08-29 · Codex · M7.3 프론트 확장 API 표면은 아직 없다 (사용자 결정)

- **결정**: M7.3 의 계약은 `ExtensionInfo.web_entry_url` 을 그대로 동적
  `import()` 해 **모듈을 평가하는 데까지**다. 확장이 호출할 수 있는 export,
  등록 함수, 위젯 마운트·해제 수명주기는 아직 정의하지 않는다. 특히 테스트가
  모듈 평가를 관찰하려고 쓴 `globalThis` 부수효과를 사실상의 공개 API 로
  굳히지 않는다
- **이유**: 표면을 조용히 비워 두면 노드 저작 가이드를 쓰는 에이전트가 작동
  ...
  반복하지 않기 위해, **없다는 상태 자체를 계약으로 기록**한다
- **선행 조건**: M7.5 노드 저작 가이드는 프론트 확장 API 표면을 사용자 확인 후
  `design.md` §8 과 이 결정 로그에 먼저 확정한 뒤 작성한다. 그 전에는
  `globalThis` 나 임의 DOM 부수효과를 확장 저작 방법으로 문서화하지 않는다
- **영향 범위**: `apps/web/src/extensions/loader.ts`, `docs/design.md` §8,
  `docs/roadmap.md` M7.5
- **되돌릴 수 있나**: 예 — 공개 표면을 별도 결정으로 확정하면 이 "아직 없음"
  상태를 명시적으로 대체한다
