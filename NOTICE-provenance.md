...
라이선스가 미정인 동안(→ `docs/license.md`) 이 기록이 모든 선택지를 지키는 근거가 된다.
...
**"복사 여부: 있음"이 한 번이라도 기록되면 라이선스 선택지가 붕괴한다.** 그런 항목이 생기면 즉시 소유자에게 알리고 해당 코드를 제거·재작성한다.
...
> LICENSE 파일이 생기거나 GPL/AGPL 라이선스 표기가 의존성에 들어오면 빌드가 실패한다.
...
허용적 라이선스만 사용한다. 상세 목록은 `docs/license.md` 참조.
